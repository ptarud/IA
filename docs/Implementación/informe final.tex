...
\begin{abstract}
En el presente trabajo, se estudiará el Traveling Tournament Problem (TTP), para luego analizar el nuevo problema Time Relaxed Tournament Problem (RTTP). Además de 
ésto se verá lo que se ha hecho hasta ahora, los mejores resultados y técnicas que se han usado hasta ahora para resolver ambos problemas. Luego de ésto se 
propondrá un algoritmo de búsqueda local llamado Hill Climbing con el fin de resolver el problema RTTP.
\end{abstract}
...
En el presente trabajo, se definirá lo que es TTP, para luego introducir la variante RTTP que es la que se centrará en resolver. Se definirá de maneras mas detallada el RTTP, además
...
En \cite{RSGD:25}, presenta un método de búsqueda completo para resolver el problema, usando
...
70000 y 30 respectivamente. Se hará el experimiento para todas las instancias y varias veces, ya que el algoritmo depende mucho de la solución inicial.
...
Para TTP los mejores resultados conocidos en la literatura son los que utilizan métodos con post-procesamiento en la búsqueda local (movimientos) y usan métodos incompletos para 
resolverlo, como \emph{Tabu Search}, \emph{Hill Climbing} y \emph{Simulated Annealing} o una combinación de éstos. \\

Para el RTTP los mejores resultados son los basados en programación entera y programación con restricciones. También los que usan métodos de búsqueda completa para resolver el problema, usando
Branch and Bound, metaheurísticas y programación dinámica. \\

Hill Climbing es un algoritmo de búsqueda local, que requiere de una solución inicial, por lo tanto éste depende mucho de la solución 
inicial encontrada, por lo que al correrlo se necesita correrlo varias veces. \\

Se ha introducido un parámetro \textbf{maxIters} que es el número máximo de iteraciones en el que se deja al algoritmo estar estancado en un óptimo
...
distinta solución inicial, permitiendo al algoritmo explorar más sobre el espacio de búsqueda. \\

El algoritmo depende también de la cantidad de iteraciones en las que es ejecutado, se hizo el experimento 1 para probar dicho parámetro, quedándome 
con la cantidad que arrojó buenos resultados y no demoró tanto. \\
